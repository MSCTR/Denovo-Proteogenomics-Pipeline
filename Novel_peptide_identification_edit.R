...
f1<-list.files(pattern="*Peptide_Report.txt",recursive=TRUE)
f2<-list.files(pattern="*PSM_Report.txt",recursive=TRUE)
...
for (k in 1:length(f1)) {
	data <- xmlParse("./resources/ACTG_v1.11/mapping_params.xml")
...
for (k in 1:length(f1)) {
	i<-f1[k]
	print(i)
	cmd<-paste("java -Xmx8G -Xss2m -jar ./resources/ACTG_v1.11/ACTG_mapping.jar ",i)
	system(cmd)
	}
...
f1<-list.files(pattern="^Novel_Confident_Peptides_PSMs(.*)txt.csv$",recursive=TRUE)
f2<-list.files(pattern="*ACTG_peptides_gff_combined.csv",recursive=TRUE)
for(k in 1:length(f1)) {
	r1<-read.csv(f1[k])
